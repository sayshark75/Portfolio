  ...
  return (
    ...
    >
      ...
      >
        ...
        {process.error && (
          <Flex direction={"column"} zIndex={5} justify={"center"} align={"center"} w={"100%"} minH={"100%"} aspectRatio={"16/9"} color={"darker"}>
            <Text color={"accent"} textAlign={"center"} fontSize={"56px"} fontFamily={"Poppins"} fontWeight={400}>
              <FaRegCircleXmark />
            </Text>
            <Text textAlign={"center"} fontSize={24} fontFamily={"Poppins"} mt={5} fontWeight={300}>
        ...
        {process.loading && (
          <Flex aspectRatio={"16/9"} zIndex={5} justify={"center"} align={"center"} w={"100%"} minH={"100%"}>
            <Spinner size="xl" color="accent" borderWidth={6} />
          </Flex>
        )}
        {!process.error && !process.loading && !process.success && (
          <Flex overflow={"hidden"} as={"form"} onSubmit={handleSubmit} w={"100%"} direction={"column"} gap={4}>
            <Input
              color={"text"}
              name="name"
              pl={4}
              variant={"outline"}
              fontWeight={"300"}
              fontSize={{ base: "10px", sm: "12px", md: "14px", xl: "14px", "2xl": "16px" }}
              letterSpacing={"1.5px"}
              border={"2px solid"}
              borderColor={"accent"}
      ...
      <Flex pb={"12"} mt={4} mx={"2"} gap={{ base: 2, sm: 3 }}>
        {SocialButtonsData.map((info, index) => {
          return <IconButtonComp key={`social-btn-contact-${index}`} icon={info.icon} tooltip={info.tooltip} label={info.label} link={info.link} />;
        })}
      </Flex>
    </Flex>
  );
